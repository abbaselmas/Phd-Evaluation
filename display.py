...
config = {
        ...
        "modeBarButtonsToAdd":      ["v1hovermode", "toggleSpikeLines", "hoverClosestCartesian", "hoverCompareCartesian", "zoom", "pan", "select", "lasso2d"],
        "displaylogo":              False,
        "editable":                 False
}
        ...
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    sett = dict(mode="markers+lines", marker=dict(symbol=marker_symbols[symbol_index], size=10, color=colors[color_index]),
        ...
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    sett = dict(mode="markers", marker=dict(symbol=marker_symbols[symbol_index], size=10, color=colors[color_index]),
        ...
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    sett = dict(mode="markers+lines", marker=dict(color=colors[color_index], size=10, symbol=marker_symbols[symbol_index]),
                                line=dict(color=colors[color_index], dash=line_styles[(i+j) % len(line_styles)], width=3),
                                name=legend_groupfig5, showlegend=False, hovertemplate=f"{name.split('-')[0]}:" + "<b>%{x:.3f}</b> | " + f"{name.split('-')[1]}:"+ "<b>%{y:.3f}</b>")
        ...
        for j in range(len(DescriptorsLegend)):
            for m in range(2):
                ...
                if not (result3 == 0 or np.isnan(result3)):
                    trace_match_synt_result3 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]]], y=[result3],
                                                        name=f".{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-total",
                                                        text=[f"{result3:.3f}"], marker_color=colors[color_index], showlegend=True,
                ...
                if not (result4 == 0 or np.isnan(result4)):
                    trace_match_synt_result4 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]]], y=[result4],
                                                        name=f".{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-inlier",
                                                        text=[f"{result4:.3f}"], marker_color=colors[color_index], showlegend=True,
                                                        legendgroup=f".{DetectorsLegend[i]}-{DescriptorsLegend[j]}", hovertemplate="<b>%{y:.3f}</b>")
                ...
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    if not (np.isnan(y_data).any()):
                        traces.append (go.Scatter(  x=xvals, y=y_data, mode="markers+lines",
                                                    marker=dict(symbol=marker_symbols[symbol_index], color=colors[color_index], size=10),
...
def singleMulti(data="drone",name="Precision-Recall", x=13, y=12):
    ...
    xvals = [f"Img{i}" for i in range(153, 188)] if data == "drone" else (
        ["Bahamas", "Office", "Suburban", "Building", "Construction", "Dominica", "Cadastre", "Rivaz", "Urban", "Belleview"] if data == "uav" else 
        [f"Img{i}" for i in range(653, 774, 5)])
    fig11.update_layout(title_text=f"<b>{data.upper()} - {name}</b>", title_x=0.5, title_xanchor="center", margin=dict(l=20, r=20, t=40, b=20),
                        xaxis_title=name.split('-')[0], yaxis_title=name.split('-')[1], hovermode="x", uirevision="73", xaxis=dict(range=[0, 1]), yaxis=dict(range=[0, 1]))
    color_index = 0
    symbol_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    if not (np.isnan(Rate_x).any()) and not (np.isnan(Rate_y).any()):
                        fig11trace_UAV  = go.Scatter(   x=[Rate_x], y=[Rate_y], mode="markers", marker=dict(symbol=marker_symbols[symbol_index], size=10, color=colors[color_index]),
...
def single_timing2(data="drone"):
    ...
    Rate = np.load(f"./arrays/Rate_{data}.npy")
    fig13 = go.Figure()
    fig13.update_layout(title_text=f"<b>{data.upper()} Timings</b>", title_x=0.5, title_xanchor="center", yaxis_title="1/Inlier Time", hovermode="x", xaxis=dict(autorange=False, range=[0, 1]), margin=dict(l=20, r=20, t=60, b=20))
    color_index = 0
    symbol_index = 0
    traces = []
    for i in range(len(DetectorsLegend)):
...
def rep_err(data="drone"):
    ...
    Rate = np.load(f"./arrays/Rate_{data}.npy")
    fig14 = go.Figure()
    fig14.update_layout(title_text=f"<b>Reprojection Error: {data}</b>", title_x=0.5, title_xanchor="center", xaxis_title="pixel", yaxis_title="1/Inlier Time", hovermode="x", margin=dict(l=20, r=20, t=60, b=20), uirevision="73")
    color_index = 0
    symbol_index = 0
    traces = []
    for i in range(len(DetectorsLegend)):
