...
def synthetic(name='Precision', rate=13):
    ...
    fig1 = make_subplots(   rows=2, cols=2, subplot_titles=['Intensity changing I+b', 'Intensity changing Ixc', 'Scale changing', 'Rotation changing'],
                            shared_xaxes=False, shared_yaxes=False, horizontal_spacing=0.05, vertical_spacing=0.07, y_title=f"{name}")
    fig1.update_layout(title_text=f"Synthetic Dataset - {name}", title_x=0.45)
    fig1.update_layout(margin=dict(l=60, r=0, t=60, b=25))
            ...
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    if not (np.isnan(Rate2_I2).any()):   
                        fig1trace_I2    = go.Scatter(x=val_c, y=Rate2_I2, mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               showlegend=False, legendgroup=legend_groupfig1)
                        fig1.add_trace(fig1trace_I2, row=1, col=2)
                    if not (np.isnan(Rate2_S).any()):              
                        fig1trace_Scale = go.Scatter(x=scale, y=Rate2_S,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               showlegend=False, legendgroup=legend_groupfig1)
                        fig1.add_trace(fig1trace_Scale,  row=2, col=1)
                    if not (np.isnan(Rate2_R).any()):
                        fig1trace_Rot   = go.Scatter(x=rot,   y=Rate2_R,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               showlegend=False, legendgroup=legend_groupfig1)
    ...
    fig1.write_html(f"./html/synthetic_{name}.html")
...
def syntheticPR(name='Precision-Recall', x=13, y=12):
    ...
    fig1 = make_subplots(   rows=2, cols=2, subplot_titles=['Intensity changing I+b', 'Intensity changing Ixc', 'Scale changing', 'Rotation changing'],
                            shared_xaxes=False, shared_yaxes=False, horizontal_spacing=0.05, vertical_spacing=0.07, x_title='Recall', y_title='Precision')
    fig1.update_layout(title_text=f"Synthetic Dataset - {name}", title_x=0.45)
    fig1.update_layout(margin=dict(l=60, r=0, t=60, b=60))
...
def synthetic_timing():
    ...
    fig25.update_layout(title_text=f"Synthetic Data Timings", title_x=0.5)
    fig25.update_layout(margin=dict(l=20, r=0, t=60, b=25), barmode='stack', height=2600)
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for m in range(2):
                ...
                if m == 0: # Brute Force
                    if not np.isnan(result3):
                        trace_match_synt_result3 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            ...
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                    ...
                    if not np.isnan(result4):
                        trace_match_synt_result4 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            ...
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig25.add_trace(trace_match_synt_result4, row=2, col=1)
                ...
                if m == 1: # FLANN
                    if not np.isnan(result3):
                        trace_match_synt_result3 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            ...
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig25.add_trace(trace_match_synt_result3, row=3, col=1)
                        trace_match_synt_result3.showlegend = False
                        fig25.add_trace(trace_match_synt_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_synt_result4 = go.Bar(  x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
    ...
    for i in range(len(DetectorsLegend)):
        ...
        trace_detect_synt = go.Bar(x=[DetectorsLegend[i]], y=[result], name=DetectorsLegend[i], showlegend=True, text=[f'{result:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]))
    ...
    for j in range(len(DescriptorsLegend)):
        ...
        trace_descr_synt = go.Bar(x=[DescriptorsLegend[j]], y=[result2], name=DescriptorsLegend[j], showlegend=True, text=[f'{result2:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]))
    ...
    for descriptor in DescriptorsLegend:
        ...
        dropdown_descriptors.append(dict(label=descriptor, method="update", args=[{"visible": visible}, {"title": f"Filtered by Descriptor: {descriptor}"}]))
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig25.data)},{"title": "Synthetic Dataset - {name}"}])
    fig25.update_layout(
        updatemenus=[
            dict(type="buttons", buttons=[reset_button], x=1.10, y=0),
    ...
    fig25.write_html("./html/synthetic_timing.html")
...
def oxford(name='Precision', rate=13):
    ...
    fig2 = make_subplots(   rows=3, cols=3, subplot_titles=['Graf(Viewpoint)', 'Bikes(Blur)', 'Boat(Zoom + Rotation)', 'Leuven(Light)', 'Wall(Viewpoint)', 'Trees(Blur)', 'Bark(Zoom + Rotation)', 'UBC(JPEG)', 'Overall'],
                            shared_xaxes=False, shared_yaxes=False, horizontal_spacing=0.04, vertical_spacing=0.06, y_title=f"{name}")
    fig2.update_layout(title_text=f"Oxford Affine Dataset - {name}", title_x=0.45)
    fig2.update_layout(margin=dict(l=60, r=0, t=60, b=25))
    fig2.update_yaxes(range=[0, 1])
    x = ["Img2", "Img3", "Img4", "Img5", "Img6"]
    fig2.update_layout( xaxis  = dict(tickmode = 'array', tickvals = x),xaxis2 = dict(tickmode = 'array', tickvals = x),xaxis3 = dict(tickmode = 'array', tickvals = x),
                        xaxis4 = dict(tickmode = 'array', tickvals = x),xaxis5 = dict(tickmode = 'array', tickvals = x),xaxis6 = dict(tickmode = 'array', tickvals = x),
                        xaxis7 = dict(tickmode = 'array', tickvals = x),xaxis8 = dict(tickmode = 'array', tickvals = x),xaxis9 = dict(tickmode = 'array', tickvals = x))
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    if not (np.isnan(Rate_Graf).any()):
                        fig2trace_Graf   = go.Scatter(x=x, y=Rate_Graf,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name=legend_groupfig2, legendgroup=legend_groupfig2, showlegend=True)
                        fig2.add_trace(fig2trace_Graf, row=1, col=1)
                    if not (np.isnan(Rate_Bikes).any()):
                        fig2trace_Bikes  = go.Scatter(x=x, y=Rate_Bikes,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Bikes, row=1, col=2)
                    if not (np.isnan(Rate_Boat).any()):
                        fig2trace_Boat   = go.Scatter(x=x, y=Rate_Boat,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Boat, row=1, col=3)
                    if not (np.isnan(Rate_Leuven).any()):
                        fig2trace_Leuven = go.Scatter(x=x, y=Rate_Leuven, mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Leuven, row=2, col=1)
                    if not (np.isnan(Rate_Wall).any()):
                        fig2trace_Wall   = go.Scatter(x=x, y=Rate_Wall,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Wall, row=2, col=2)
                    if not (np.isnan(Rate_Trees).any()):
                        fig2trace_Trees  = go.Scatter(x=x, y=Rate_Trees,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Trees, row=2, col=3)
                    if not (np.isnan(Rate_Bark).any()):
                        fig2trace_Bark   = go.Scatter(x=x, y=Rate_Bark,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Bark, row=3, col=1)
                    if not (np.isnan(Rate_Ubc).any()):
                        fig2trace_Ubc    = go.Scatter(x=x, y=Rate_Ubc,    mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Ubc, row=3, col=2)
                    fig2trace_overall = go.Scatter(x=x, y=np.nanmean([Rate_Graf, Rate_Bikes, Rate_Boat, Rate_Leuven, Rate_Wall, Rate_Trees, Rate_Bark, Rate_Ubc], axis=0), mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='', legendgroup=legend_groupfig2, showlegend=False)
    ...
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig2.data)},{"title": "Synthetic Dataset - {name}"}])
    ...
    fig2.write_html(f"./html/oxford_{name}.html")
...
def oxfordPR(name='Precision-Recall', x=13, y=12):
    ...
    fig2 = make_subplots(   rows=3, cols=3, subplot_titles=['Graf(Viewpoint)', 'Bikes(Blur)', 'Boat(Zoom + Rotation)', 'Leuven(Light)', 'Wall(Viewpoint)', 'Trees(Blur)', 'Bark(Zoom + Rotation)', 'UBC(JPEG)', 'Overall'],
                            shared_xaxes=False, shared_yaxes=False, horizontal_spacing=0.06, vertical_spacing=0.06, x_title='Recall', y_title='Precision')
    fig2.update_layout(title_text=f"Oxford Affine Dataset - {name}", title_x=0.45)
            ...
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
                for m in range(2): # Matcher 0: BruteForce 1: FlannBased
                    ...
                    if not (np.isnan(Rate_Bikes_x).any()) and not (np.isnan(Rate_Bikes_y).any()):
                        fig2trace_Bikes  = go.Scatter(x=Rate_Bikes_x, y=Rate_Bikes_y,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Bikes, row=1, col=2)
                    if not (np.isnan(Rate_Boat_x).any()) and not (np.isnan(Rate_Boat_y).any()):
                        fig2trace_Boat   = go.Scatter(x=Rate_Boat_x, y=Rate_Boat_y,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Boat, row=1, col=3)
                    if not (np.isnan(Rate_Leuven_x).any()) and not (np.isnan(Rate_Leuven_y).any()):
                        fig2trace_Leuven = go.Scatter(x=Rate_Leuven_x, y=Rate_Leuven_y, mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Leuven, row=2, col=1)
                    if not (np.isnan(Rate_Wall_x).any()) and not (np.isnan(Rate_Wall_y).any()):
                        fig2trace_Wall   = go.Scatter(x=Rate_Wall_x, y=Rate_Wall_y,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Wall, row=2, col=2)
                    if not (np.isnan(Rate_Trees_x).any()) and not (np.isnan(Rate_Trees_y).any()):
                        fig2trace_Trees  = go.Scatter(x=Rate_Trees_x, y=Rate_Trees_y,  mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Trees, row=2, col=3)
                    if not (np.isnan(Rate_Bark_x).any()) and not (np.isnan(Rate_Bark_y).any()):
                        fig2trace_Bark   = go.Scatter(x=Rate_Bark_x, y=Rate_Bark_y,   mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Bark, row=3, col=1)
                    if not (np.isnan(Rate_Ubc_x).any()) and not (np.isnan(Rate_Ubc_y).any()):
                        fig2trace_Ubc    = go.Scatter(x=Rate_Ubc_x, y=Rate_Ubc_y,    mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='',               legendgroup=legend_groupfig2, showlegend=False)
                        fig2.add_trace(fig2trace_Ubc, row=3, col=2)
                    fig2trace_overall = go.Scatter( x=np.nanmean([Rate_Graf_x, Rate_Bikes_x, Rate_Boat_x, Rate_Leuven_x, Rate_Wall_x, Rate_Trees_x, Rate_Bark_x, Rate_Ubc_x], axis=0),
                                                    ...
                                                    mode='lines+markers', marker_symbol=raw_symbols[color_index%56], marker_size=7, line=dict(color=color, dash=style), name='', legendgroup=legend_groupfig2, showlegend=False)
                    fig2.add_trace(fig2trace_overall, row=3, col=3)
                    color_index += 1
    # Dropdown for Detector
    dropdown_detectors = []
...
def oxford_timing():
    ...
    fig55 = make_subplots(  rows=5, cols=2, subplot_titles=['Average 1k Detect time',
                                                            ...
                                                            'Average 1k Inlier time (Detect + Descript + Match(BF+FL) + RANSAC)',],
                            specs=[[{}, {}], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None]], horizontal_spacing=0.05, vertical_spacing=0.07)
    fig55.update_layout(title_text="Oxford Affine Dataset - Timing", title_x=0.5)
    fig55.update_layout(margin=dict(l=20, r=0, t=60, b=25), barmode='stack', height=2600)
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for m in range(2):           
                ...
                if m == 0: # Brute Force
                    if not np.isnan(result3):
                        trace_match_oxford_result3 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig55.add_trace(trace_match_oxford_result3, row=2, col=1)
                        trace_match_oxford_result3.showlegend = False
                        fig55.add_trace(trace_match_oxford_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_oxford_result4 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-inlier',
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig55.add_trace(trace_match_oxford_result4, row=2, col=1)
                        trace_match_oxford_result4.showlegend = False
                        fig55.add_trace(trace_match_oxford_result4, row=5, col=1)
                if m == 1: # FLANN
                    if not np.isnan(result3):
                        trace_match_oxford_result3 = go.Bar(x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig55.add_trace(trace_match_oxford_result3, row=3, col=1)
                        trace_match_oxford_result3.showlegend = False
                        fig55.add_trace(trace_match_oxford_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_oxford_result4 = go.Bar(x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
    ...
    for descriptor in DescriptorsLegend:
        ...
        dropdown_descriptors.append(dict(label=descriptor, method="update", args=[{"visible": visible}, {"title": f"Filtered by Descriptor: {descriptor}"}]))
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig55.data)},{"title": "Synthetic Dataset - {name}"}])
    fig55.update_layout(
        updatemenus=[
            dict(type="buttons", buttons=[reset_button], x=1.10, y=0),
            dict(buttons=dropdown_detectors,   x=1.05, y=1.01),
    ...
    fig55.write_html("./html/oxford_timing.html")
...
def drone(name='Precision', rate=13):
    ...
    fig551 = go.Figure()
    fig551.update_layout(title_text=f"Drone Data - {name}", title_x=0.5)
    fig551.update_layout(margin=dict(l=60, r=0, t=60, b=25))
    xvals = [f'Img{i}' for i in range(153, 188)]
    fig551.update_layout(xaxis = dict(tickmode = 'array', tickvals = xvals))
    fig551.update_yaxes(range=[0, 1])
    fig551.update_yaxes(title_text=f"{name}")
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
    ...
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig551.data)},{"title": "Synthetic Dataset - {name}"}])
    ...
    fig551.write_html(f"./html/drone_{name}.html")
...
def dronePR(name='Precision-Recall', x=13, y=12):
    ...
    fig551 = go.Figure()
    fig551.update_layout(title_text=f"Drone Data - {name}", title_x=0.5)
    fig551.update_layout(margin=dict(l=60, r=0, t=60, b=60))
    xvals = [f'Img{i}' for i in range(153, 188)]
    fig551.update_layout(xaxis = dict(tickmode = 'array', tickvals = xvals))
    fig551.update_yaxes(range=[0, 1])
    fig551.update_xaxes(range=[0, 1])
    fig551.update_yaxes(title_text="Precision")
    fig551.update_xaxes(title_text="Recall") 
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
...
def drone_timing():
    ...
    fig56 = make_subplots(  rows=5, cols=2, subplot_titles=['Average 1k Detect time',
                                                            ...
                                                            'Average 1k Inlier time (Detect + Descript + Match(BF+FL) + RANSAC)',],
                            specs=[[{}, {}], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None]], horizontal_spacing=0.05, vertical_spacing=0.07)
    fig56.update_layout(title_text="Drone Data Timing", title_x=0.5)
    fig56.update_layout(margin=dict(l=20, r=20, t=60, b=25), barmode='stack', height=2600)
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for m in range(2):
                ...
                if m == 0: # Brute Force
                    if not np.isnan(result3):
                        trace_match_drone_result3 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'total')
                        fig56.add_trace(trace_match_drone_result3, row=2, col=1)
                        trace_match_drone_result3.showlegend = False
                        fig56.add_trace(trace_match_drone_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_drone_result4 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            name=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'inlier',
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'inlier')
                        fig56.add_trace(trace_match_drone_result4, row=2, col=1)
                        trace_match_drone_result4.showlegend = False
                        fig56.add_trace(trace_match_drone_result4, row=5, col=1)
                if m == 1: # FLANN
                    if not np.isnan(result3):
                        trace_match_drone_result3 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'total')
                        fig56.add_trace(trace_match_drone_result3, row=3, col=1)
                        trace_match_drone_result3.showlegend = False
                        fig56.add_trace(trace_match_drone_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_drone_result4 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            ...
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=DetectorsLegend[i] + '-' + DescriptorsLegend[j] + '-' + Matcher[m] + '-' + 'inlier')
                        fig56.add_trace(trace_match_drone_result4, row=3, col=1)
                        trace_match_drone_result4.showlegend = False
                        fig56.add_trace(trace_match_drone_result4, row=5, col=1)
                color_index += 28
    # Detector time
    ...
    for i in range(len(DetectorsLegend)):
        ...
        trace_detect_drone = go.Bar(x=[DetectorsLegend[i]], y=[result], name=DetectorsLegend[i], showlegend=True, text=[f'{result:.4f}'], textposition='auto')
    ...
    for j in range(len(DescriptorsLegend)):
        ...
        trace_descr_drone = go.Bar(x=[DescriptorsLegend[j]], y=[result2], name=DescriptorsLegend[j], showlegend=True, text=[f'{result2:.4f}'], textposition='auto')
    ...
    for descriptor in DescriptorsLegend:
        ...
        dropdown_descriptors.append(dict(label=descriptor, method="update", args=[{"visible": visible}, {"title": f"Filtered by Descriptor: {descriptor}"}]))
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig56.data)},{"title": "Synthetic Dataset - {name}"}])
    fig56.update_layout(
        updatemenus=[
            dict(type="buttons", buttons=[reset_button], x=1.10, y=0),
            dict(buttons=dropdown_detectors,   x=1.05, y=1.01),
    ...
    fig56.write_html("./html/drone_timing.html")
...
def uav(name='Precision', rate=13):
    ...
    fig552 = go.Figure()
    fig552.update_layout(title_text=f"UAV Data - {name}", title_x=0.5)
    fig552.update_layout(margin=dict(l=60, r=0, t=60, b=25))
    xvals = ['Bahamas', 'Office', 'Suburban', 'Building', 'Construction', 'Dominica', 'Cadastre', 'Rivaz', 'Urban', 'Belleview']
    fig552.update_layout(xaxis = dict(tickmode = 'array', tickvals = xvals))
    fig552.update_yaxes(range=[0, 1])
    fig552.update_yaxes(title_text=f"{name}") 
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
    ...
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig552.data)},{"title": "Synthetic Dataset - {name}"}])
    ...
    fig552.write_html(f"./html/uav_{name}.html")
...
def uavPR(name='Precision-Recall', x=13, y=12):
    ...
    fig552 = go.Figure()
    fig552.update_layout(title_text=f"UAV Data - {name}", title_x=0.5)
    fig552.update_layout(margin=dict(l=60, r=20, t=60, b=25))
    xvals = ['Bahamas', 'Office', 'Suburban', 'Building', 'Construction', 'Dominica', 'Cadastre', 'Rivaz', 'Urban', 'Belleview']
    fig552.update_layout(xaxis = dict(tickmode = 'array', tickvals = xvals))
    fig552.update_yaxes(range=[0, 1])
    fig552.update_xaxes(range=[0, 1])
    fig552.update_yaxes(title_text="Precision")
    fig552.update_xaxes(title_text="Recall") 
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for c3 in range(2): # Normalization Type 0: L2 1: Hamming
...
def uav_timing():
    ...
    fig57 = make_subplots(  rows=5, cols=2, subplot_titles=['Average 1k Detect time',
                                                            ...
                                                            'Average 1k Inlier time (Detect + Descript + Match(BF+FL) + RANSAC)',],
                            specs=[[{}, {}], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None], [{"colspan": 2}, None]], horizontal_spacing=0.05, vertical_spacing=0.07)
    fig57.update_layout(title_text="UAV Data - Timing", title_x=0.5)
    fig57.update_layout(margin=dict(l=20, r=20, t=60, b=25), barmode='stack', height=2600)
    color_index = 0
    for i in range(len(DetectorsLegend)):
        for j in range(len(DescriptorsLegend)):
            for m in range(2):
                ...
                if m == 0:
                    if not np.isnan(result3):
                        trace_match_uav_result3 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig57.add_trace(trace_match_uav_result3, row=2, col=1)
                        trace_match_uav_result3.showlegend = False
                        fig57.add_trace(trace_match_uav_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_uav_result4 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-inlier',
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig57.add_trace(trace_match_uav_result4, row=2, col=1)
                        trace_match_uav_result4.showlegend = False
                        fig57.add_trace(trace_match_uav_result4, row=5, col=1)
                if m == 1:
                    if not np.isnan(result3):
                        trace_match_uav_result3 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result3],
                                                            name=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}-{Matcher[m]}-total',
                                                            text=[f'{result3:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig57.add_trace(trace_match_uav_result3, row=3, col=1)
                        trace_match_uav_result3.showlegend = False
                        fig57.add_trace(trace_match_uav_result3, row=4, col=1)
                    if not np.isnan(result4):
                        trace_match_uav_result4 = go.Bar( x=[[DetectorsLegend[i]], [DescriptorsLegend[j]], [Matcher[m]]], y=[result4],
                                                            ...
                                                            text=[f'{result4:.4f}'], textposition='auto', marker=dict(color = colors[color_index%num_combinations]),
                                                            showlegend=True, legendgroup=f'{DetectorsLegend[i]}-{DescriptorsLegend[j]}')
                        fig57.add_trace(trace_match_uav_result4, row=3, col=1)
                        trace_match_uav_result4.showlegend = False
                        fig57.add_trace(trace_match_uav_result4, row=5, col=1)
                color_index += 28
    ...
    for i in range(len(DetectorsLegend)):
        ...
        trace_detect_uav = go.Bar(x=[DetectorsLegend[i]], y=[result], name=DetectorsLegend[i], showlegend=True, text=[f'{result:.4f}'], textposition='auto')
    ...
    for j in range(len(DescriptorsLegend)):
        ...
        trace_descr_uav = go.Bar(x=[DescriptorsLegend[j]], y=[result2], name=DescriptorsLegend[j], showlegend=True, text=[f'{result2:.4f}'], textposition='auto')
    ...
    for descriptor in DescriptorsLegend:
        ...
        dropdown_descriptors.append(dict(label=descriptor, method="update", args=[{"visible": visible}, {"title": f"Filtered by Descriptor: {descriptor}"}]))
    reset_button = dict(label="Reset", method="update", args=[{"visible": [True] * len(fig57.data)},{"title": "Synthetic Dataset - {name}"}])
    fig57.update_layout(
        updatemenus=[
            dict(type="buttons", buttons=[reset_button], x=1.10, y=0),
            dict(buttons=dropdown_detectors,   x=1.05, y=1.01),
    ...
    fig57.write_html("./html/uav_timing.html")
